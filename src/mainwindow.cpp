...
GTextEdit *currentEditorWidget = NULL;
...
void MainWindow::newTab(){
    if (ui->tabWidget->count() < 50){
        ui->tabWidget->addTab(new GTextEdit, "New File");
...
{
    QString licence = "Project Page: https://github.com/Wazecode/gide \n\n"
            "GIDE - Guild's Integrated Development Environment\n"
            "Version: 0.7\n"
            "Copyright (C) 2024\n\n"
    ...
    QMessageBox::information(this, tr("About GIDE"), licence,QMessageBox::Close);
}
...
void MainWindow::setTabWidth(int width){
    ...
    for(int i = 0; i < ui->tabWidget->count(); i++){
        ui->tabWidget->setCurrentIndex(i);
        currentEditorWidget->setTabStopDistance(width * metrics.horizontalAdvance(' ') );
    }
